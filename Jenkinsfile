            ...
            steps {
                script {
                    ...
                    docker.withRegistry("https://${ECR_REPO}", "${ECR_CREDENTIALS_ID}") {
                        ...
                        // 도커 컨테이너 실행
                        sh "docker run -d -e BACKEND_URL=${BACKEND_URL} -e NEXT_PUBLIC_SOCKET_URL=${NEXT_PUBLIC_SOCKET_URL} -e RECOMMEND_AI_URL=${RECOMMEND_AI_URL} -e ALPHA_VANTAGE_API_KEY=${ALPHA_VANTAGE_API_KEY} -e HUGGINGFACE_API_KEY=${HUGGINGFACE_API_KEY} -e ALPHA_VANTAGE_BACKEND_URL=${ALPHA_VANTAGE_BACKEND_URL} --name todayfin-fe -p 3000:3000 ${ECR_REPO}:latest"
